...
class Cart extends Component {
  ...
  render() {
    const { carts, deleteProductFromLocalCart } = this.props
    return (
            ...
            carts.map(product => (
              <div className="cart-row" key={product.id}>
                <div className="cart-item cart-column">
                  <img className="cart-item-image" alt="" src={`http://localhost:3005/${product.thumbnail}`} width="100" height="100" />
                ...
                <div className="cart-quantity cart-column">
                  ...
                  <button className="btn btn-danger" type="button" onClick={() => {deleteProductFromLocalCart(product.id)}} >删除</button>
                </div>
              </div>
            ))
          }
