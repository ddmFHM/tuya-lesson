...
1. 创建 store 关联 reducer(参数为reduer)
2. reducer
  - 初始化数据
  - 根据数据 state 和 action 操作数据 （按照type以及payload）
3. 根组件/子组件需要共享的父组件 使用 Provider 包裹并传入 store
4. 将触发函数归总在 actions 文件内导出
5. 通过 mapStateToProps 以及 mapDispatchToProps 将数据和action映射到props内
  - mapStateToProps
    ```jsx
    ...
    ```
  - mapDispatchToProps 使用 bindActionCreators 来简化生成action触发函数（bindActionCreators返回的是一个对象）
    ```jsx
      const mapDispatchToProps = (dispatch) => ({
        ...bindActionCreators(contentActions, dispatch)
      })
    ```
6. 通过 connect 函数关联数据并返回一个新的组件导出
...
使用 applyMiddleware 注册 thunk 为中间件，使得 action 可以为一个函数，在函数内进行异步操作，然后 dispatch 触发 action 操作

## redux-sage 异步解决方案

1. 使用 saga 中间件
2. 定义 saga 文件，监听 action (有异步请求的)
3. 执行完异步操作，触发 action (操作数据，同步)
